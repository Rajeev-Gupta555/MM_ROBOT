...
class Diff_drive():
    ...
    def move(self, cmd_vector):
        ...
        th = th + 2*math.pi if th < 0 else th
        alpha = alpha + 2*math.pi if th < 0 else alpha
        err_th1 = (th-alpha)
        if abs(err_th1) > abs(2*math.pi - err_th1):
            err_th1 -= math.pi*2
        ...
        # print(th, err_th1)
        data = Twist()
        data.linear.x = self.cmd_vel[0] if self.cmd_vel[2] < self.max_angular_vel else self.max_angular_vel/4
        data.angular.z = self.cmd_vel[2] if self.cmd_vel[0] < self.max_linear_vel else self.max_linear_vel/4
