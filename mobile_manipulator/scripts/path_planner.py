...
class Path_Planner():
    def __init__(self, x_min, y_min, x_max, y_max, function=''):
        ...
        self.only_point_movement = False
    ...
    def move_to(self, x_goal, y_goal):
        ...
        if not self.only_point_movement:
            # to track the line joining the goal and the bot
            if dist > self.step_size:
                m = (y_goal-y0)/(x_goal-x0) if x_goal!=x0 else 1e100 * (-1 if y_goal < y0 else 1) 
                c = y0 - m * x0
            ...
            print("Moving to ", self.x_goal, self.y_goal)
            while(dist > 3*self.step_size):
                # print(self.drive.posn)
                vector = [0.0, 0.0]
                try:
                    vector = [-(self.drive.posn[0]-self.x_goal)/dist, -(self.drive.posn[1]-self.y_goal)/dist]
        ...
        # if self.only_point_movement : plt.legend()        
        # plt.show()
        self.drive.stop()
        return True
